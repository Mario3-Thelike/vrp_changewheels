		...
		if vehicle ~= 0 then
			...
			if closestTire ~= nil then
				if IsVehicleTyreBurst(vehicle, closestTire.tireIndex, 0) == false then
					...
					Draw3DText(closestTire.bonePos.x, closestTire.bonePos.y, closestTire.bonePos.z, "~w~Cracked wheel!")
				end
			end
		end
	end
		...
		if DoesEntityExist(vehicle) then
			...
			TriggerServerEvent('hjalte:takeTheTire')
				...
				Citizen.CreateThread(function()
					...
					TriggerEvent("pNotify:SendNotification", {text = "Changing tires!", type = "info", timeout = 5000, layout = "centerLeft"})
					...
					if CurrentAction ~= nil then
						...
						if closestTire ~= nil then
							...
							TriggerEvent("pNotify:SendNotification", {text = "You are now done changing the tire!", type = "success", timeout = 2000, layout = "centerLeft"})
						end
					end
